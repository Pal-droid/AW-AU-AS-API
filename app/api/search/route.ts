...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[v0] Raw results from scrapers:`, {
      animeworldResults: JSON.stringify(animeworldResults),
      animesaturnResults: JSON.stringify(animesaturnResults),
    });
    ...
    console.log(`[v0] AnimeWorld results:`, JSON.stringify(awResults));
    console.log(`[v0] AnimeSaturn results:`, JSON.stringify(asResults));

    // Combine and deduplicate results
    const unifiedResults = detectDuplicates(awResults, asResults);
    console.log(`[v0] Unified results after deduplication:`, JSON.stringify(unifiedResults));
    console.log(`[v0] Final unified results count: ${unifiedResults.length}`);

    return NextResponse.json(unifiedResults);
  } catch (error) {
    console.error(`[v0] Exception in search endpoint: ${error}`);
    return NextResponse.json(
